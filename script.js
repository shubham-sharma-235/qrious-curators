...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const nxWords = document.querySelectorAll('.nx-word');
...
window.addEventListener("scroll",()=>{
    ...
    const step = maxScroll / contentSlides.length;

    let index = Math.floor(scrollTop / step);
    ...
    btn.addEventListener('click', () => {
      ...
      if (filter === 'all') {
        ...
        filteredSlides = [...slides].filter(slide =>
          slide.classList.contains(filter)
        );
